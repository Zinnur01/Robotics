...
cap = cv2.VideoCapture("../../Resources/cards-vid.mp4")
imgTarget = cv2.imread("../../Resources/cards.jpg")
myVid = cv2.VideoCapture("../../Resources/crossroad.mp4")
...
while True:
    ...
    imgWebcam = cv2.resize(imgWebcam, None, fx=0.4, fy=0.4)
    ...
    for m, n in matches:
        ...
        if len(good) > 20:
            ...
            # the matching matrix
            matrix, mask = cv2.findHomography(srcPts, dstPts, cv2.RANSAC, 4)
            ...
            imgAug = cv2.bitwise_and(imgAug, imgAug, mask=cv2.bitwise_not(maskNew))
