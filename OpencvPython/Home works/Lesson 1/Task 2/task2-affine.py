...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP:
        groups[selectedGroupIndex] = [rectStartPos, (x, rectStartPos[1]), (x, y)]
        isMouseDrag = False
    ...
    for i, points in enumerate(groups):
        ...
        if isMouseDrag and i == selectedGroupIndex:
            ...
            if points:
                ...
                fromMatrix = np.array([points[0], points[1], points[2]]).astype(np.float32)
                toMatrix = np.array([
                    [0, 0],
                    [width, 0],
                    [width, height],
                ]).astype(np.float32)
                matrix = cv2.getAffineTransform(fromMatrix, toMatrix)
                ...
                oppositePoint = (points[0][0] + points[2][0] - points[1][0], points[0][1] + points[2][1] - points[1][1])

                cv2.line(img_copy, points[2], oppositePoint, lineColor, 2)
                cv2.line(img_copy, oppositePoint, points[0], lineColor, 2)
